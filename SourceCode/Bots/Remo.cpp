...
ARemo::ARemo()
{
	...
	ConstructorHelpers::FObjectFinder<UStaticMesh> MeshObj(TEXT("/Game/RemoBot_V1"));	//get mesh
	...
	//RootComponent = meesh;
	meesh->SetupAttachment(RootComponent);												//Attach to NPC:Remo
...
//Repair buildings
void ARemo::repair_Implementation(AActor* t) {
	UE_LOG(LogTemp, Warning, TEXT("Going"));
	this->target = t;
	//GetWorld()->GetNavigationSystem()->SimpleMoveToLocation(Controller, FVector(0, 0, 0));
	GetWorld()->GetNavigationSystem()->SimpleMoveToActor(Controller, t);
...
// Called every frame
void ARemo::Tick(float DeltaTime)
{
...
void ARemo::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
