...
ARemo_V2::ARemo_V2()
{
	...
	//Set Icon texture
	ConstructorHelpers::FObjectFinder<UTexture2D> icon(TEXT("/Game/Texture/Icons/Remo_V2Icon"));
	...
	ConstructorHelpers::FObjectFinder<UStaticMesh> MeshObj(TEXT("/Game/RemoBot_V2"));	//get mesh
	...
	meesh->CustomDepthStencilValue = STENCIL_FRIENDLY_OUTLINE;							//colour set
																						...
																						//RootComponent = meesh;
	meesh->SetupAttachment(RootComponent);												//Attach to NPC:Remo_V2
...
//Repair buildings
void ARemo_V2::repair_Implementation(AActor* t) {
	UE_LOG(LogTemp, Warning, TEXT("Going"));
	this->target = t;
	//GetWorld()->GetNavigationSystem()->SimpleMoveToLocation(Controller, FVector(0, 0, 0));
	GetWorld()->GetNavigationSystem()->SimpleMoveToActor(Controller, t);
...
// Called every frame
void ARemo_V2::Tick(float DeltaTime)
{
	...
	if (state == harvesting) {
		...
		if (carryAmount >= MAX_CARRY) {
			...
			case 2:
				...
				break;
			case 3:
				(Cast<AMyCameraPawn>(GetWorld()->GetFirstPlayerController()->GetPawn()))->updateCurrentMetal(x);
				break;
			default:
				break;
			}
...
void ARemo_V2::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
