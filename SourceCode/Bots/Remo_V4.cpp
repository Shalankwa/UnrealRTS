...
ARemo_V4::ARemo_V4()
{
	...
	//Set Icon texture
	ConstructorHelpers::FObjectFinder<UTexture2D> icon(TEXT("/Game/Texture/Icons/Remo_V4Icon"));
	...
	ConstructorHelpers::FObjectFinder<UStaticMesh> MeshObj(TEXT("/Game/RemoBot_V4"));	//get mesh
	...
	meesh->CustomDepthStencilValue = STENCIL_FRIENDLY_OUTLINE;							//colour set
																						...
																						//RootComponent = meesh;
	meesh->SetupAttachment(RootComponent);												//Attach to NPC:Remo_V4
...
//Repair buildings
void ARemo_V4::repair_Implementation(AActor* t) {
	UE_LOG(LogTemp, Warning, TEXT("Going"));
	this->target = t;
	//GetWorld()->GetNavigationSystem()->SimpleMoveToLocation(Controller, FVector(0, 0, 0));
	GetWorld()->GetNavigationSystem()->SimpleMoveToActor(Controller, t);
...
// Called every frame
void ARemo_V4::Tick(float DeltaTime)
{
	...
	if (state == harvesting) {
		...
		if (carryAmount >= MAX_CARRY) {
			...
			case 1:
				(Cast<AMyCameraPawn>(GetWorld()->GetFirstPlayerController()->GetPawn()))->updateCurrentLiquid(x * 2);
				break;
			case 2:
				(Cast<AMyCameraPawn>(GetWorld()->GetFirstPlayerController()->GetPawn()))->updateCurrentWood(x * 2);
				break;
			case 3:
...
void ARemo_V4::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
